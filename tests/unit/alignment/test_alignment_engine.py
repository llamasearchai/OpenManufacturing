...
from openmanufacturing.core.alignment.alignment_engine import AlignmentEngine, AlignmentParameters, MockPosition as CoreMockPosition # If MockPosition is from engine
...
def mock_motion_controller():
    ...
    async def move_relative(x=0, y=0, z=0, speed=None):
        ...
        dist_sq = current_position_state["x"]**2 + current_position_state["y"]**2 + current_position_state["z"]**2
        optical_power_state["power"] = -2.0 * np.exp(-dist_sq / 2.0) - 1.0 # Peak at -1dBm at (0,0,0)
        return True
    
    async def move_absolute(x=None, y=None, z=None, speed=None):
        if x is not None: current_position_state["x"] = x
        if y is not None: current_position_state["y"] = y
        if z is not None: current_position_state["z"] = z
        dist_sq = current_position_state["x"]**2 + current_position_state["y"]**2 + current_position_state["z"]**2
...
async def test_coarse_alignment(alignment_engine, mock_motion_controller, mock_calibration_profile):
    mock_detect_fiber_patcher = patch(f'{VISION_PROCESSING_PATH}.detect_fiber_position', new_callable=AsyncMock)
    mock_detect_waveguide_patcher = patch(f'{VISION_PROCESSING_PATH}.detect_chip_waveguide', new_callable=AsyncMock)

    with mock_detect_fiber_patcher as mock_detect_fiber, \
...
async def test_fine_alignment_success(alignment_engine, mock_motion_controller):
    ...
    assert abs(result["final_position"]["x"]) < alignment_engine.parameters.position_tolerance_um * 2 # Allow some margin
    assert abs(result["final_position"]["y"]) < alignment_engine.parameters.position_tolerance_um * 2
    assert abs(result["final_position"]["z"]) < alignment_engine.parameters.position_tolerance_um * 2
    assert result["final_power_dbm"] > initial_power # Power should have improved
    assert result["iterations_performed"] > 0
    assert result["duration_ms"] > 0

@pytest.mark.asyncio
async def test_full_alignment_process(alignment_engine, mock_motion_controller, mock_calibration_profile):
    mock_detect_fiber_patcher = patch(f'{VISION_PROCESSING_PATH}.detect_fiber_position', new_callable=AsyncMock)
    mock_detect_waveguide_patcher = patch(f'{VISION_PROCESSING_PATH}.detect_chip_waveguide', new_callable=AsyncMock)
    ...
    with mock_detect_fiber_patcher as mock_detect_fiber, \
        ...
        assert abs(fine_res["final_position"]["x"]) < alignment_engine.parameters.position_tolerance_um * 2
        assert abs(fine_res["final_position"]["y"]) < alignment_engine.parameters.position_tolerance_um * 2
        assert abs(fine_res["final_position"]["z"]) < alignment_engine.parameters.position_tolerance_um * 2
...
async def test_alignment_stop_request(alignment_engine, mock_motion_controller):
    ...
    with mock_detect_fiber_patcher as mock_detect_fiber, \
        ...
        assert "stopped" in result["final_status_message"].lower() or "cancelled" in result["final_status_message"].lower() 