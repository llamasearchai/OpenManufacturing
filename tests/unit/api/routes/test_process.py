...
async def test_create_process_instance_success(
    client, mock_workflow_manager, mock_active_user_instance
):
    template_id = "template-123"
    batch_id = "batch-abc"
    process_id = str(uuid.uuid4())
    # current_time = datetime.utcnow() # Unused variable
    ...
    mock_workflow_manager.create_process_instance.assert_called_once_with(
        ...
        metadata={
            "key": "value",
            "created_by_user_id": mock_active_user_instance.id,
            "created_by_username": mock_active_user_instance.username,
        },
    )
