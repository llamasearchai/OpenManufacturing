...
async def test_login_for_access_token_incorrect_password(client, mock_db_session):
    ...
    mock_user_db = DBUser(id=1, username=test_username, email="test@example.com", hashed_password=hashed_password, is_active=True)
...
async def test_login_for_access_token_inactive_user(client, mock_db_session):
    ...
    test_password = "password123"
    hashed_password = get_password_hash(test_password)
    mock_user_db = DBUser(id=1, username=test_username, email="inactive@example.com", hashed_password=hashed_password, is_active=False)
...
async def test_register_user_already_exists(client, mock_db_session):
    ...
    mock_existing_user = DBUser(id=2, username=existing_username, email="exists@example.com", hashed_password="somehash")
...
async def test_read_users_me(client, mock_get_current_active_user_override, mock_active_user):
    ...
    response = client.get("/api/auth/me") # No token needed as TestClient handles dependency overrides
