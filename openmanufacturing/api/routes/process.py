...
async def create_process_template_endpoint(
    ...
    query = select(DBWorkflowTemplate).where(
        DBWorkflowTemplate.name == request.name, DBWorkflowTemplate.version == request.version
    )
    ...
    new_template = DBWorkflowTemplate(
        ...
        steps=request.steps,
        created_by=current_user.id,  # User.id is int
    )
...
async def delete_process_template_endpoint(
    ...
    ),  # Consider admin only: get_current_admin_user
):
    ...
    active_instances_query = select(DBProcessInstance).where(
        ...
        DBProcessInstance.state.in_(
            [ProcessState.PENDING.name, ProcessState.RUNNING.name, ProcessState.PAUSED.name]
        ),
    )
    ...
    if active_instances_exist.scalars().first():
        raise HTTPException(
            status_code=400,
            detail="Cannot delete template: It is used by one or more active process instances.",
        )
...
async def create_process_instance_endpoint(
    ...
    try:
        ...
        instance = await process_manager.create_process_instance(
            template_id=request.template_id, batch_id=request.batch_id, metadata=instance_metadata
        )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during process instance creation."
        )
...
async def start_process_instance_endpoint(
    ...
    try:
        ...
        await process_manager.start_process(
            process_id
        )  # This should be an async operation that kicks off the process
        ...
        if status_after_start_request["state"] not in [
        ...
        ValueError
    ) as e:  # Typically if process_id not found, or invalid state transition by manager
        logger.warning(f"ValueError starting process {process_id}: {str(e)}")
        raise HTTPException(
            status_code=400, detail=str(e)
        )  # 400 for bad request (e.g. already running/done)
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error while starting process {process_id}."
        )
...
async def get_process_instance_status_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error getting process status for {process_id}.",
        )
...
async def list_process_instances_endpoint(
    ...
    try:
        query = select(DBProcessInstance).options(
            selectinload(DBProcessInstance.template)
        )  # Eager load template for name
        if batch_id:
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error while listing process instances."
        )
...
async def pause_process_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error pausing process {process_id}."
        )
...
async def resume_process_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error resuming process {process_id}."
        )
...
async def abort_process_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal server error aborting process {process_id}."
        )
