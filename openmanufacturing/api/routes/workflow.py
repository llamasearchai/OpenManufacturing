...
async def list_workflow_templates(
    ...
    query = (
        select(DBWorkflowTemplate)
        .order_by(DBWorkflowTemplate.name, DBWorkflowTemplate.version)
        .offset(skip)
        .limit(limit)
    )
...
async def update_workflow_template(
    ...
    if request.name or request.version:
        ...
        if existing.scalar_one_or_none():
            raise HTTPException(
                status_code=409,
                detail="Another workflow template with the same name and version already exists",
            )
...
async def execute_workflow(
    ...
    try:
        ...
        instance = await process_manager.create_process_instance(
            template_id=request.template_id, metadata=metadata
        )
