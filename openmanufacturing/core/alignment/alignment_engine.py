...
class AlignmentEngine:
    ...
    async def perform_coarse_alignment(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            logger.debug(f"Fiber position: {fiber_position}, Waveguide position: {waveguide_position}")
            ...
            logger.debug(f"Movement deltas: ({delta_x}, {delta_y}, {delta_z}), " 
    ...
    async def perform_fine_alignment(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if strategy == "gradient":
                ...
                logger.warning(f"Invalid optimization strategy '{strategy}', using gradient descent")
                success, result = await self._gradient_descent_alignment()
            ...
            if result["success"]:
                ...
                logger.warning(f"Fine alignment did not achieve target power using {strategy} strategy")
            ...
            for _ in range(self.parameters.max_iterations // len(step_sizes)):
                ...
                for axis in axes:
                    ...
                    if gradient_magnitude < 0.001:
                        logger.debug(f"Gradient magnitude very small ({gradient_magnitude}), may be at local optimum")
                        break
                    
                    # Move in the direction of the gradient
                    move_deltas = {axis: step_size * gradient[axis] / gradient_magnitude for axis in axes}
                ...
                if abs(best_power - last_best_power) < 0.01:
                    ...
                    if convergence_count >= 3:  # 3 iterations without significant improvement
                        logger.debug(f"Converged after {iterations} iterations (no significant improvement)")
                        break
                else:
                    convergence_count = 0
    ...
    async def _spiral_search_alignment(self) -> Tuple[bool, Dict]:
        ...
        for i in range(1, spiral_points):
            ...
            if new_power > best_power:
                ...
                # Check if we've reached target power
                if best_power >= self.parameters.optical_power_threshold:
                    logger.info(f"Reached target power threshold ({best_power} dBm) during spiral search")
                    break
