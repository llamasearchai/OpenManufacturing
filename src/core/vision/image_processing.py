...
class ImageProcessor:
    ...
    def __init__(self, camera_id: str = "default", config_path: str = "config/vision.json", 
    ...
    async def capture_images(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.simulation_mode:
            ...
            cv2.line(top_image, (fiber_x - 100, fiber_y), (fiber_x + 100, fiber_y), (200, 200, 200), 2)
            ...
            cv2.line(side_image, (fiber_x - 100, fiber_z), (fiber_x + 100, fiber_z), (200, 200, 200), 2)
            # Add bright spot at the tip
            ...
            cv2.rectangle(top_image, (chip_x - 150, chip_y - 100), (chip_x + 150, chip_y + 100), (150, 150, 150), 2)
            # Side view - chip outline
            cv2.rectangle(side_image, (chip_x - 150, chip_z - 20), (chip_x + 150, chip_z + 20), (150, 150, 150), 2)
    ...
    async def detect_fiber_position(self) -> Position3D:
        ...
        lines_top = cv2.HoughLinesP(edges_top, 1, np.pi/180, hough_threshold, 
                                   minLineLength=min_line_length, maxLineGap=max_line_gap)
        
        lines_side = cv2.HoughLinesP(edges_side, 1, np.pi/180, hough_threshold, 
                                    minLineLength=min_line_length, maxLineGap=max_line_gap)
        ...
        if side_confidence == 0:
            ...
            if max_val > 200:  # If there's a bright spot
                side_x = max_loc[0] + gray_side.shape[1]//2  # Adjust for right half offset
                side_z = max_loc[1]
                side_confidence = 0.5
    ...
    async def detect_chip_waveguide(self) -> Position3D:
        ...
        logger.debug(f"Detected waveguide position: ({x}, {y}, {z}) with confidence {confidence:.2f}")
        return Position3D(x=x, y=y, z=z, confidence=confidence)
