...
class MotionController:
    ...
    def __init__(self, 
        ...
        self.type = ControllerType[controller_type.upper()] if not simulation_mode else ControllerType.SIMULATED
        self.port = port
        ...
        logger.info(f"Motion controller initialized (type: {self.type.name}, simulation: {self.simulation_mode})")
    ...
    async def disconnect(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.error(f"Failed to disconnect from {self.controller_type.name} motion controller")
                return False
    ...
    async def move_absolute(self, x: Optional[float] = None, y: Optional[float] = None, 
                           z: Optional[float] = None, speed: float = 1.0) -> bool:
        """
    ...
    async def move_relative(self, x: float = 0.0, y: float = 0.0, z: float = 0.0, 
    ...
    async def set_optimal_position(self, x: float, y: float, z: float, peak_power: float = -2.0):
        ...
        if self.simulation_mode:
            ...
            logger.info(f"Set simulated optimal position to {self._optimal_position}, peak power: {peak_power} dBm")
    ...
    async def close(self):
        ...
        logger.info(f"Motion controller connection closed")