...
class AlignmentService:
    ...
    async def align_device(self, request_id: str, device_id: str,
        ...
        result_payload = {
            ...
            "parameters": parameters.model_dump() if parameters else AlignmentParameters().model_dump(),
        ...
        asyncio.create_task(self._perform_alignment(request_id, device_id, parameters, process_id, metadata)) # Pass all necessary info
        
        logger.info(f"Scheduled alignment for device {device_id}, request ID: {request_id}")
        return request_id
    
    async def _perform_alignment(self, request_id: str, device_id: str, 
        ...
        try:
            ...
            if not result["success"] and not result.get("error"):
                 final_result_data["error"] = result.get("fine_alignment", {}).get("error", "Alignment did not reach target power or failed.")
            ...
            if result["success"]:
                logger.info(f"Alignment successful for request {request_id}. Power: {final_result_data['optical_power_dbm']:.2f} dBm, Position: {final_result_data['position']}")
            else:
                logger.warning(f"Alignment failed or did not reach target power for request {request_id}. Error: {final_result_data['error']}")
            ...
            error_result_data = {
                ...
                "duration_ms": duration_ms,
                "error": str(e),
                "trajectory": self.active_alignments.get(request_id, {}).get("trajectory", []) # Keep previous trajectory if any
            }
            async with self._lock:
                if request_id in self.active_alignments:
    ...
    async def _update_alignment_result(self, request_id: str, status: str, success: bool,
                                    position: Optional[Dict[str, float]] = None,
                                    optical_power_dbm: Optional[float] = None, # Made Optional
                                    duration_ms: int = 0,
                                    iterations: int = 0,
                                    error: Optional[str] = None,
                                    trajectory: Optional[List[Dict[str,Any]]] = None) -> None: # Added Trajectory
        """
    ...
    def cancel_alignment(self, request_id: str) -> bool:
        ...
        if not active_alignment_data:
            logger.warning(f"Alignment request {request_id} not found or not active for cancellation.")
            return False
    ...
    async def _process_cancellation(self, request_id: str, alignment_data: Dict[str, Any]):
        ...
        async with self._lock:
            if request_id in self.active_alignments: # Re-check if it's still active
            ...
            elif request_id in self.completed_alignments and self.completed_alignments[request_id]["status"] != "cancelled":
                ...
                self.completed_alignments[request_id].update({
                    "status": "cancelled",
                    "error": self.completed_alignments[request_id].get("error", "") + "; " + cancellation_error_message,
                    "timestamp": current_time.isoformat()
                })
                final_cancel_data = self.completed_alignments[request_id] # for saving
            else: # Already cancelled or unknown
                logger.warning(f"Alignment {request_id} was not active or already processed during cancellation.")
                return
        ...
        try:
            async with get_db_session() as session:
                ...
                if device_id:
                    ...
                    if not device:
                        logger.warning(f"Device {device_id} not found in database for alignment result {result_data['request_id']}")
                else:
                    logger.warning(f"Device ID missing in alignment result {result_data['request_id']}, cannot save to DB fully.")
                ...
                if process_id:
                    ...
                    if not process:
                        logger.warning(f"Process {process_id} not found in database for alignment result {result_data['request_id']}")
