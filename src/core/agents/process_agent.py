...
from ..process.workflow_manager import WorkflowManager, ProcessState, ProcessInstance
...
class ProcessAgent(BaseAgent):
    ...
    async def create_process(self, 
        ...
        try:
            ...
            self.state.process_metrics[process.id] = {
                ...
                "status_history": [{"status": ProcessState.PENDING.name, "timestamp": datetime.now().isoformat()}],
                "duration_ms": 0,
                "steps_completed": 0,
    ...
    async def get_process_metrics(self, process_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "active_processes": len([p for p, s in self.state.active_processes.items() 
                                    if s in [ProcessState.RUNNING.name, ProcessState.PAUSED.name]]),
            "completed_processes": len([p for p, s in self.state.active_processes.items() 
                                      if s == ProcessState.COMPLETED.name]),
            "failed_processes": len([p for p, s in self.state.active_processes.items() 
                                   if s == ProcessState.FAILED.name]),
            "avg_duration_ms": 0,
            "success_rate": 0
        }
        
        # Calculate average duration and success rate
        completed = [m for p, m in self.state.process_metrics.items() 
        ...
        if completed:
            ...
            total_finished = len([p for p, s in self.state.active_processes.items() 
                ...
                for process_id in process_ids:
                    try:
                        ...
                        if current_state != self.state.active_processes.get(process_id):
                            ...
                            if process_id in self.state.process_metrics:
                                self.state.process_metrics[process_id]["status_history"].append(
                                    {"status": current_state, "timestamp": datetime.now().isoformat()}
                                )
                            
                            logger.info(f"Process {process_id} state changed: {old_state} -> {current_state}")
                            
                            # Handle completed processes
                            if current_state == ProcessState.COMPLETED.name:
                                # Calculate duration
                                if process_id in self.state.process_metrics:
                                    history = self.state.process_metrics[process_id]["status_history"]
                                    start_time = None
                                    ...
                                    if start_time:
                                        ...
                                        self.state.process_metrics[process_id]["duration_ms"] = duration_ms
                                        self.state.process_metrics[process_id]["completed_at"] = end_time.isoformat()
                            
                            # Handle failed processes
                            elif current_state == ProcessState.FAILED.name:
                                if process_id in self.state.process_metrics:
                                    self.state.process_metrics[process_id]["error_count"] += 1
                                    self.state.process_metrics[process_id]["failed_at"] = datetime.now().isoformat()
    ...
    async def _collect_process_metrics(self):
        ...
        while not self.stopping:
            try:
                ...
                active_process_ids = [p for p, s in self.state.active_processes.items() 
                    ...
                    try:
                        ...
                        if process_id in self.state.process_metrics:
                            ...
                            if "step_results" in status:
                                completed_steps = sum(1 for step, result in status["step_results"].items() 
                                                   if result == "completed")
                                self.state.process_metrics[process_id]["steps_completed"] = completed_steps
